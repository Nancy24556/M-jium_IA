...
// Garantit que le compte admin (ADMIN_EMAIL) a bien le rôle 'admin' et un
// accès Premium permanent — appelé à chaque inscription/connexion pour que
// ça reste vrai même si la base a été créée avant l'ajout de cette règle.
function ensureAdminRole(userId, email) {
  if (email.toLowerCase() !== ADMIN_EMAIL) return;
  db.prepare(
    `UPDATE users SET role = 'admin', premium_until = ?, subscription_status = 'premium' WHERE id = ?`
  ).run(new Date(Date.now() + 100 * 365 * 24 * 60 * 60 * 1000).toISOString(), userId);
}
