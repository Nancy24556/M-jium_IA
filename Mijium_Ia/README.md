# Mï — Assistant de guidage pas à pas
...
Mï est un assistant **généraliste** : elle ne connaît pas une activité à l'avance, elle la découvre en discutant avec l'utilisateur (sport, cuisine, langue, bricolage, révisions, projet pro… n'importe quoi), puis le guide **une étape à la fois**, sans jamais tout donner d'un coup.
...
```
mii-assistant/
├── backend/                 → serveur (API + IA + base de données + facturation)
│   ├── server.js             point d'entrée
│   ├── db.js                 base SQLite (utilisateurs, sessions, messages)
│   ├── prompts.js             prompt système de Mï
...
│   ├── routes/                routes API (auth, chat, billing, rewards, user)
│   └── .env.example           variables à configurer
├── frontend/                 → application web (interface utilisateur)
│   ├── index.html / css / js  écrans : connexion, chat, progrès, profil
...
|---|---|---|
| Guidage pas à pas par IA | `prompts.js`, `agent.js` | Prompt système qui interdit à Mï de tout donner d'un coup, mémoire de conversation par session |
| Évaluation initiale | `prompts.js` | Mï découvre l'activité et le niveau dès le premier message |
...
| Abonnement Premium | `billing.js`, `routes/billing.js` | Mode simulation par défaut, bascule vers Stripe réel si vous renseignez des clés |
| Parrainage (7 jours offerts x2) | `rewards.js` | Lien unique par utilisateur, crédité à la première conversion du filleul |
| Gamification / paliers | `rewards.js` | Séries de jours consécutifs, thèmes/bonus débloqués |
...
Ouvrez ensuite **http://localhost:3000** dans votre navigateur : le serveur sert à la fois l'API et l'interface web. Créez un compte, discutez avec Mï, testez l'abonnement (en mode simulation, il s'active immédiatement sans carte bancaire) et le lien de parrainage.
...
2. **Politique de confidentialité** : obligatoire, avec une URL publique renseignée dans la Play Console (Mï collecte des e-mails, mots de passe et messages de conversation).
...
- Export du guide de connaissance métier si vous voulez spécialiser Mï sur un domaine précis (remettre un fichier `knowledge/guide_activite.md` injecté dans `prompts.js`).
