// js/api.js — Petit client HTTP pour parler au backend Mïjium.
...
const Api = {
  ...
  renameSession: (sessionId, title) =>
    apiRequest(`/api/chat/session/${sessionId}`, { method: "PATCH", body: { title } }),
  ...
  adminTransactions: () => apiRequest("/api/admin/transactions"),
  adminUsers: () => apiRequest("/api/admin/users"),
  adminGrantPremium: (userId, days) =>
    apiRequest(`/api/admin/users/${userId}`, { method: "PATCH", body: { grantPremiumDays: days } }),
  adminRevokePremium: (userId) =>
    apiRequest(`/api/admin/users/${userId}`, { method: "PATCH", body: { revokePremium: true } }),
