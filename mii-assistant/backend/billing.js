...
async function subscribe(userId, { days = 30 } = {}) {
  ...
  if (stripe && process.env.STRIPE_PRICE_ID_PREMIUM) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        process.env.STRIPE_SUCCESS_URL || (appUrl ? `${appUrl}/?premium=success` : "https://example.com/success"),
      cancel_url:
        process.env.STRIPE_CANCEL_URL || (appUrl ? `${appUrl}/?premium=cancel` : "https://example.com/cancel"),
